...
-- Run this entire script in Supabase SQL Editor to update everything at once.
...
-- We use a DO block to handle the enum update safely
DO $$
BEGIN
    -- Check if 'dept_manager_approved' exists in the enum, if not, we need to update it
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'dept_manager_approved' AND enumtypid = 'request_status'::regtype) THEN
        ...
        -- Create new type with all needed values
...
-- Set default warehouse email (Update as needed)
...
-- 5. Helper function to drop policies if they exist (to avoid errors)
DO $$
BEGIN
    -- Drop policies for approval_logs if they exist
